...
- Typed data layer in `web/src/cms/`: `getPageData`, `getGlobalData`, and `getStaticPaths` wrap a `createPayloadSDK` factory with a module-scoped cache that is gated off in dev so CMS edits propagate without an Astro restart.
